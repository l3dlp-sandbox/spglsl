...
async function _setupServer() {
  ...
  app.post("/spglsl/glsl", async (req, res) => {
    ...
    try {
      ...
      console.log(`output:\n${result.output === null ? chalk.redBright("null") : chalk.cyan(result.output)}\n`);

      if (result.output && !result.infoLog.hasErrors()) {
        const validation = await spglslAngleCompile({
          mainFilePath: "0",
          mainSourceCode: result.output,
          compileMode: "Validate",
          language: shaderType,
        });
        if (validation.infoLog.hasErrors()) {
          console.error(chalk.redBright("VALIDATION FAILED!", validation.infoLog));
        }
      }
      ...
      const error = (err instanceof Error ? err : new Error(err as any)) as any;
      error.__url = req.url;
